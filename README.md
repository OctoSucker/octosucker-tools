...
OctoSucker Skill 接口包 - 定义 Skill 开发的标准接口和工具注册机制

## 概述

`octosucker-skill` 是 OctoSucker AI Agent 平台的 Skill 开发基础库。它定义了：

- **Tool 系统**: `Tool`, `ToolHandler`, `ToolRegistry` - 工具定义和注册机制
- **Skill 接口**: `SkillLifecycle`, `SkillMetadata`, `SkillRegisterFunc` - Skill 生命周期和注册接口
- **Skill 注册机制**: 自动发现和加载已注册的 Skill
